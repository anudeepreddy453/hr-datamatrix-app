...
const Layout = () => {
  ...
  const navigation = [
    ...
    ...(user?.role === 'admin' || user?.role === 'hr_manager' ? [{ name: 'Succession Plans', href: '/succession-plans', icon: TrendingUpIcon }] : []),
    { name: 'Analytics', href: '/analytics', icon: BarChartIcon },
    // Only show Access Requests for HR users
    ...(user?.role === 'admin' || user?.role === 'hr_manager' ? [{ name: 'Access Requests', href: '/access-requests', icon: UserCheckIcon }] : []),
    // Only show Audit Trail for HR users
    ...(user?.role === 'admin' || user?.role === 'hr_manager' ? [{ name: 'Audit Trail', href: '/audit-trail', icon: HistoryIcon }] : []),
    // Add Users (Admin) section only for admin users
    ...(user?.role === 'admin' ? [{ name: 'Users (Admin)', href: '/admin/users', icon: ShieldIcon }] : [])
  ];
